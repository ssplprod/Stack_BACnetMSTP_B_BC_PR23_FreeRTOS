...
void Clear_InitiateQ_Data(InitiateInfo_t *pstInitiateQ)
{
    ...
    {
#if (defined(BACDEL_SER_DS_RP_A) || (defined BASIC_A_SIDE))
        case SERVICE_SUPPORTED_READ_PROPERTY :
        {

			#ifdef OLD_RP_WP_INTERFACE
            /* Free memory for Property value in IP Argument */
            Clear_Property_Value(pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
                m_stServiceChoice.m_stRP_Response.m_stPropertyValue.pstNextPropVal);
            pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
                m_stServiceChoice.m_stRP_Response.m_stPropertyValue.pstNextPropVal = NULL;

			/* Free the string pointer */
			if(NULL != pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stRP_Response.m_pu8PropValue)
			{
				OSAL_Free(pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
					m_stServiceChoice.m_stRP_Response.m_pu8PropValue,
			...
			#ifdef NEW_RP_WP_INTERFACE
			/* clear allocated memories */
			Clear_PropVal_AsPer_DataType(pstInitiateQ->m_stIpArgs.m_stNPDUData.
				m_stAPDUData.m_stServiceChoice.m_stRP_Response.m_eDataType,
				&pstInitiateQ->m_stIpArgs.m_stNPDUData.
				m_stAPDUData.m_stServiceChoice.m_stRP_Response.m_pvPropVal);
			#endif

            /* Free the Response from Argument list */
            memset(&(pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
                m_stServiceChoice.m_stRP_Response), 0, sizeof(rp_response_t));

            /* Free the request Q slot */
        ...
        break;
#endif /* (defined(BACDEL_SER_DS_RP_A) || defined(BACDEL_SER_DS_WP_A)) */

#if (defined(BACDEL_SER_DS_WP_A) || (defined BASIC_A_SIDE))
        case SERVICE_SUPPORTED_WRITE_PROPERTY:
        {
            BACNET_CONF_DATA *pstWpata = NULL;

            /* Free memory for Property value in IP Argument */
            /*Clear_Property_Value(pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
                m_stServiceChoice.m_stWP_Request.m_stPropertyValue.pstNextPropVal);
            pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
                m_stServiceChoice.m_stWP_Request.m_stPropertyValue.pstNextPropVal = NULL;*/

            pstWpata = pstInitiateQ->m_ReqProcInfo.m_stProcessData.m_stAPDU.m_pvServiceRequestData;
			pstInitiateQ->m_ReqProcInfo.m_stProcessData.m_stAPDU.m_pvServiceRequestData = NULL;

            if(NULL != pstWpata)
            {
				/* free memory used to save property value */
				#ifdef OLD_RP_WP_INTERFACE
                Clear_Property_Value((BACNET_PROPERTY_VALUE *)pstWpata->pvReadPropValue);
				pstWpata->pvReadPropValue = NULL;
				#elif defined NEW_RP_WP_INTERFACE
				/* clear allocated memories */
				Clear_PropVal_AsPer_DataType(pstWpata->eData_Type ,&pstWpata->pvReadPropValue);
				#endif

				/* free WP-A request data */
				OSAL_Free(pstWpata,	__FILE__, __FUNCTION__, __LINE__);
            }
        }
        break;
...
#if (defined BACDEL_SER_DS_RPM_A || defined BASIC_A_SIDE)
        case SERVICE_SUPPORTED_READ_PROP_MULTIPLE :
        {
			#ifdef OLD_RP_WP_INTERFACE
			/* Free memory for Property value in 1st IP Argument (its static) */
            Clear_Property_Value(pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
                m_stServiceChoice.m_stRPM_Response.m_stPropertyValue.pstNextPropVal);
			/* making pointer null */
			pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.m_stServiceChoice.
				m_stRPM_Response.m_stPropertyValue.pstNextPropVal = NULL;
            /* Free the string pointer */
			if(NULL != pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stRPM_Response.m_pu8PropValue)
			{
				OSAL_Free(pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
					m_stServiceChoice.m_stRPM_Response.m_pu8PropValue,
					__FILE__, __FUNCTION__, __LINE__);
			...
			/* clear allocated memories */
			Clear_PropVal_AsPer_DataType(pstInitiateQ->m_stIpArgs.m_stNPDUData.
				m_stAPDUData.m_stServiceChoice.m_stRPM_Response.m_eDataType,
				&pstInitiateQ->m_stIpArgs.m_stNPDUData.
				m_stAPDUData.m_stServiceChoice.m_stRPM_Response.m_pvPropVal);
			#endif
#if 1
            /* Free the Response from IP Argument list from next (Malloced) */
            Free_Argument_Memory(pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.
                m_stServiceChoice.m_stRPM_Response.m_pstRPMNextElem,
                pstInitiateQ->m_stIpArgs.m_stNPDUData.m_stAPDUData.m_eServiceSupport);

            /*Clear the Initiate Q request buffers */
            Free_Service_Memory(pstInitiateQ->m_ReqProcInfo.m_stProcessData.m_stAPDU.
                m_pvServiceRequestData);
#endif
            pstInitiateQ->m_ReqProcInfo.m_stProcessData.m_stAPDU.m_pvServiceRequestData = NULL;
        }
	...
	bacnetip_arguments_t *pstIpArguments)
{
    ...
    void *pvObject = NULL;
    PROP_ACCESS_TYPE eType = BACNET_DEFAULT;
#endif
	void *pvData = NULL;
	...
	#ifdef OLD_RP_WP_INTERFACE
    /* NOTE : this code is added to return access type in response struct */
    /* get virtual device pointer */
    //pVirtualDev = pstRspQueue->m_pvReqDevStruct;
    pVirtualDev = Find_In_Host_Device_List(pstIpArguments->m_u32DeviceID);
	#endif
	...
	#if (STACK_FALLBACK_FOR_RPM)
	/* check for RPM fallback */
    if(pstInitQData->m_bRPMError == true &&
        pstRspQueue->m_stProcessData.m_ePDUType != PDU_TYPE_COMPLEX_ACK )
	{
		/* change PDU type to complex ack */
        pstIpArguments->m_stNPDUData.m_stAPDUData.m_ePduType = PDU_TYPE_COMPLEX_ACK;
	}
	#endif
    ...
    switch(eServicesSupported)
    {
        ...
        {
			/* local variables */
			BACNET_CONF_DATA rpData = {0};
			BACNET_PROPERTY_VALUE stPropVal = {0};
			int8_t *pvPropVal = NULL;
			uint32_t u32StrLen = 0;

            /* Copy the service request for futher processing */
            memcpy(&rpData, pstRspQueue->m_stProcessData.m_stAPDU.m_pvServiceRequestData
                    , sizeof(BACNET_CONF_DATA));

			#ifdef OLD_RP_WP_INTERFACE
            /* NOTE : this code is added to return access type in response struct */
            if(NULL != pVirtualDev)
                /* find the object */
                pvObject = Find_Object(rpData.eObjectType, rpData.u32ObjectInstance, pVirtualDev);
            if(NULL != pvObject)
            {
                /* get access type */
                eType = GetAsignPropAccess(rpData.eObjectType,
                    rpData.eObjectProperty, pvObject, eType, FALSE, NULL);
                if(READ_ONLY == eType || COMMANDABLE_PROP == eType)
                {
                    /* check if still writable */
                    if(Is_Property_Writable(rpData.eObjectProperty, rpData.eObjectType,
                        pvObject, pVirtualDev))
                        eType = READ_WRITE;
                    else;
                }
            }
            ...
            /* Handle special case for RPM-RP service and fill RPM service parameters */
            if(true != pstInitQData->m_bRPMError && true != pstInitQData->m_bRPObjListError)
            {
                /* Fill response of normal RP-A service in IP Arguments */
                rp_response_t *pstRP_Response = NULL;

                pstRP_Response = &(pstIpArguments->m_stNPDUData.m_stAPDUData.
                                    m_stServiceChoice.m_stRP_Response);

				pstRP_Response->m_eObjectType = rpData.eObjectType;
				pstRP_Response->m_u32ObjectInstance = rpData.u32ObjectInstance;
				pstRP_Response->m_eObjectProperty = rpData.eObjectProperty;
                pstRP_Response->m_u32ArrayIndex = rpData.u32ArrayIndex;
				pstRP_Response->m_u8ArrayIndexPresent = rpData.bArrIndxPresent;

				Decode_A_Side_Data((void *)&rpData, &pvPropVal,&stPropVal, SERVICE_SUPPORTED_READ_PROPERTY,
					(int32_t *)&u32StrLen, &pvData,
					&eDataType, &eErrCode, &eErrClass);

				#ifdef OLD_RP_WP_INTERFACE
				/* save the response data */
				pstRP_Response->m_eAccessType = eType;
				pstRP_Response->m_pu8PropValue = pvPropVal;
				pstRP_Response->m_u32StringLen = u32StrLen;
				memcpy(&pstRP_Response->m_stPropertyValue, &stPropVal,
					sizeof(BACNET_PROPERTY_VALUE));
				#elif defined NEW_RP_WP_INTERFACE
				/* save the data type & property value pointer */
				pstRP_Response->m_eDataType = eDataType;
				pstRP_Response->m_pvPropVal = pvData;
				#endif

                /* check & fill error if required */
                if(MAX_BACNET_ERROR_CODE != eErrCode)
                {
                    pstIpArguments->m_stNPDUData.m_stAPDUData.m_ePduType = PDU_TYPE_ERROR;
                    Fill_Error_Response(eErrCode, eErrClass, SERVICE_SUPPORTED_READ_PROPERTY,
                        &pstIpArguments->m_stNPDUData.m_stAPDUData.
                        m_stServiceChoice.m_stErrorResponse);
                    eErrCode = MAX_BACNET_ERROR_CODE;
                }
            }

			#if (STACK_FALLBACK_FOR_RPM)
            else if(true == pstInitQData->m_bRPMError && false == pstInitQData->m_bRPObjListError)
            {
                /* Fill Response of RPM-RP-A in IP arguments */
                rpm_response_t  *pstRpmStart = NULL;
                rpm_response_t  *pstRpmTemp = NULL;

                pstRpmStart = pstInitQData->m_pstRpmIPArgs;
//              pstRpmStart = pstInitQData->m_stIpArgs.m_stNPDUData.m_stAPDUData.
//                                    m_stServiceChoice.m_stRPM_Response;

                if(NULL == pstRpmStart)
                {
                    /* Allocate memory to Application arguments */
                    pstInitQData->m_pstRpmIPArgs = OSAL_Malloc(sizeof(rpm_response_t),
                                                    __FILE__, __FUNCTION__, __LINE__);
                    pstRpmStart = pstInitQData->m_pstRpmIPArgs;
                    pstRpmStart->m_pstRPMNextElem = NULL;
                }
                else
                {
                    while(NULL != pstRpmStart)
                    {
                        pstRpmTemp = pstRpmStart;
                        pstRpmStart = pstRpmStart->m_pstRPMNextElem;
                    }
                    /* Allocate memory and add Link to RPM arguments */
                    pstRpmTemp->m_pstRPMNextElem = OSAL_Malloc(sizeof(rpm_response_t),
                                                    __FILE__, __FUNCTION__, __LINE__);

                    pstRpmStart = pstRpmTemp->m_pstRPMNextElem;
                    pstRpmStart->m_pstRPMNextElem = NULL;
                }

                /* Set Error class and Error code to -1 to indiacate successful response */
				pstRpmStart->m_eErrorClass = -1;
				pstRpmStart->m_eErrorCode = -1;

				pstRpmStart->m_eObjectType = rpData.eObjectType;
				pstRpmStart->m_u32ObjectInstance = rpData.u32ObjectInstance;
				pstRpmStart->m_eObjectProperty = rpData.eObjectProperty;
                pstRpmStart->m_u32ArrayIndex = rpData.u32ArrayIndex;
				pstRpmStart->m_u8ArrayIndexPresent = rpData.bArrIndxPresent;

                /* Decode the application RPM data to appropriate type */
				Decode_A_Side_Data((VOID *)&rpData, &pvPropVal,
                    &stPropVal, SERVICE_SUPPORTED_READ_PROPERTY,
					&u32StrLen, &pvData,
					&eDataType, &eErrCode, &eErrClass);

				#ifdef OLD_RP_WP_INTERFACE
				/* save the response data */
				pstRpmStart->m_eAccessType = eType;
				pstRpmStart->m_pu8PropValue = pvPropVal;
				pstRpmStart->m_u32StringLen = u32StrLen;
				memcpy(&pstRpmStart->m_stPropertyValue, &stPropVal,
					sizeof(BACNET_PROPERTY_VALUE));
				#elif defined NEW_RP_WP_INTERFACE
				/* save the data type & property value pointer */
				pstRpmStart->m_eDataType = eDataType;
				pstRpmStart->m_pvPropVal = pvData;
				#endif

                /* check & fill error if required */
                if(MAX_BACNET_ERROR_CODE != eErrCode)
                {
                    Fill_Error_Response(eErrCode, eErrClass, SERVICE_SUPPORTED_READ_PROP_MULTIPLE,
                        pstRpmStart);
                    eErrCode = MAX_BACNET_ERROR_CODE;
                }
            }
			...
			#if (STACK_FALLBACK_FOR_RP_OBJECT_LIST)
			/* validate RP fallback error flag */
			else if(false == pstInitQData->m_bRPMError && true == pstInitQData->m_bRPObjListError)
			{
				/* local variables */
				uint8_t u8TempBuf[50] = {0};
				uint32_t u32Len = 0;
				uint32_t u32CopyLen = 0;
				bool bCopyError = false;

				/* increment index for next request */
				pstInitQData->m_u32CurrentIndex++;

				/* get object-list count in global */
				if(BACNET_ZERO == rpData.u32ArrayIndex)
				{
					/* allocate memory to RP response */
					pstInitQData->m_pstRpObjListIPArgs = OSAL_Malloc(sizeof(rp_response_t),
						__FILE__, __FUNCTION__, __LINE__);
					/* validate memory */
					if(NULL == pstInitQData->m_pstRpObjListIPArgs)
					{
						#if(defined DEBUG_PRINTF && DL_0)
						Print_DebugMsg(DEBUG_LEVEL0, BACDEL_MALLOC_ERROR,
						"APDU: Fill_BacnetIP_Arguments: RP-A fallback resp data malloc failed \r\n");
						#endif
						// FIXME - need to handle this, return error to application that fallback failed
						return;
					}

					/* fill response data */
					pstInitQData->m_pstRpObjListIPArgs->m_eObjectType = rpData.eObjectType;
					pstInitQData->m_pstRpObjListIPArgs->m_u32ObjectInstance = rpData.u32ObjectInstance;
					pstInitQData->m_pstRpObjListIPArgs->m_eObjectProperty = rpData.eObjectProperty;
					pstInitQData->m_pstRpObjListIPArgs->m_u32ArrayIndex = -1;
					pstInitQData->m_pstRpObjListIPArgs->m_u8ArrayIndexPresent = false;
					...
					/* check tag type */
					if(BACNET_TWO == stPropVal.m_TagType)
					{
						/* save array size in global variable */
						pstInitQData->m_u32ObjListSize = stPropVal.uValue.m_Unsigned_Int;
					}

					/* free allocated memory of property value */
					OSAL_Free(pvPropVal,  __FILE__, __FUNCTION__, __LINE__);
				}
				else
				{
					/* validate pointer value */
					if(NULL == pstInitQData->m_pu8ObjListData)
					{
						/* allocate memory for object list */
						pstInitQData->m_pu8ObjListData = (uint8_t *)OSAL_Malloc(MAX_APDU_LENGTH_ACCEPTED,
							__FILE__, __FUNCTION__, __LINE__);
						/* vallidate pointer value */
						if(NULL == pstInitQData->m_pu8ObjListData)
						{
							#if(defined DEBUG_PRINTF && DL_0)
							Print_DebugMsg(DEBUG_LEVEL0, BACDEL_MALLOC_ERROR,
							"APDU: Fill_BacnetIP_Arguments: RP-A fallback string data malloc failed \r\n");
							#endif
							// FIXME - need to handle this, return error to application that fallback failed
							return;
						}
					...
					/* decode the application RP data to appropriate type */
					Decode_A_Side_Data((VOID *)&rpData, &pvPropVal,
						&stPropVal, SERVICE_SUPPORTED_READ_PROPERTY,
						&u32StrLen, &pvData,
						&eDataType, &eErrCode, &eErrClass);

					/* add one opening bracket */
					Strncat(&u8TempBuf[u32Len], "(", 1);
					/* increase length */
					u32Len += BACNET_ONE;
					/* copy data */
					Strncat(&u8TempBuf[u32Len], pvPropVal, u32StrLen);
					/* increase length */
					u32Len += u32StrLen;
					/* add one closing bracket */
					Strncat(&u8TempBuf[u32Len], ")", 1);
					/* increase length */
					u32Len += BACNET_ONE;
					/* validate max array index */
					if(pstInitQData->m_u32ObjListSize != rpData.u32ArrayIndex)
					{
						/* add comma */
						Strncat(&u8TempBuf[u32Len], ",", 1);
						/* increase length */
					...
					/* copy object-list data */
					#ifdef SEGMENTATION_SUPPORTED
        			u32CopyLen = Memcopy_Dynamic_Mem_Expansion((void **)&pstInitQData->m_pu8ObjListData,
						&u8TempBuf[0], pstInitQData->m_u32Offset, u32Len, &pstInitQData->m_u32BufferSize,
						MAX_APDU_LENGTH_ACCEPTED);
					#else
					i32CopyLen = Memcopy(&pstInitQData->m_pu8ObjectIdBackup, &pvPropVal, pstInitQData->u32offset,
						u32Len, MAX_APDU_LENGTH_ACCEPTED);
					#endif
					/* check if data was copied or not */
					if(u32CopyLen == 0 && u32Len != 0)
					{
						/* set current array index size to greater max array index to stop fallback mechanism */
						pstInitQData->m_u32CurrentIndex = (pstInitQData->m_u32ObjListSize + 1);
						/* break the loop to send abort */
						bCopyError = true;
						break;
					}

					/* update offset value */
					pstInitQData->m_u32Offset += u32Len;
				}
			}
			#endif /* RP fallback */
        }
        ...
        {
			/* local variables */
			Bacnet_Multiple_Data_t stRpmData = {{0}};
            Bacnet_Multiple_Data_t *pstRPMData = NULL;
            rpm_response_t  *pstRpmStart = NULL;
            //rpm_response_t  *pstTemp = NULL;
			BACNET_PROPERTY_VALUE stPropVal = {0};
			int8_t *pvPropVal = NULL;
			uint32_t u32StrLen = 0;

            pstRpmStart = &(pstIpArguments->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
                            m_stRPM_Response);

            pstRpmStart->m_pstRPMNextElem = NULL;
            //pstTemp = pstRpmStart;

            /* Copy the data to RPM structure for decoding */
            memcpy((void *)&stRpmData, pstRspQueue->m_stProcessData.m_stAPDU.m_pvServiceRequestData, sizeof(Bacnet_Multiple_Data_t));

            /* Taking RPM pointer reference to avoid crash issue */
            pstRPMData = &stRpmData;

            /* Check end of properties in Response */
            while(NULL != pstRPMData)
            {
                ...
                if(NULL != pVirtualDev)
                /* find the object */
                pvObject = Find_Object(pstRPMData->m_stServiceData.eObjectType,
                    pstRPMData->m_stServiceData.u32ObjectInstance, pVirtualDev);

                /* NOTE : this code is added to return access type in response struct */
                if(NULL != pvObject)
                {
                    eType = BACNET_DEFAULT;
                    /* get access type */
                    eType = GetAsignPropAccess(pstRPMData->m_stServiceData.eObjectType,
                        pstRPMData->m_stServiceData.eObjectProperty, pvObject, eType, FALSE, NULL);
                    if(READ_ONLY == eType || COMMANDABLE_PROP == eType)
                    {
                        /* check if still writable */
                        if(Is_Property_Writable(pstRPMData->m_stServiceData.eObjectProperty,
                ...
                /* fill response */
				pstRpmStart->m_eObjectType = pstRPMData->m_stServiceData.eObjectType;
				pstRpmStart->m_u32ObjectInstance = pstRPMData->m_stServiceData.u32ObjectInstance;
				pstRpmStart->m_eObjectProperty = pstRPMData->m_stServiceData.eObjectProperty;
                pstRpmStart->m_u32ArrayIndex = pstRPMData->m_stServiceData.u32ArrayIndex;
				pstRpmStart->m_u8ArrayIndexPresent = pstRPMData->m_stServiceData.bArrIndxPresent;
				#ifdef OLD_RP_WP_INTERFACE
				pstRpmStart->m_eAccessType = eType;
				#endif

                /* Copy response paramter to Argumets */
                if(pstRPMData->m_stServiceData.bErrorStatus)
                {
                    /* copy Error class and code */
					pstRpmStart->m_eErrorClass = pstRPMData->m_stServiceData.eErrorClass;
					pstRpmStart->m_eErrorCode = pstRPMData->m_stServiceData.eErrorCode;
                }
                else
                {
                    /* Set Error class and Error code to -1 to indiacate successful response */
                    ...
                    /* Decode the application RPM data to appropriate type */
                    Decode_A_Side_Data((void *)&pstRPMData->m_stServiceData, &pvPropVal, &stPropVal,SERVICE_SUPPORTED_READ_PROP_MULTIPLE, (int32_t *)&u32StrLen,
						&pvData, &eDataType, &eErrCode, &eErrClass);
                ...
                if(NULL != pstRPMData->m_pstMDNext)
                {
                    /* Allocate memory to Application arguments */
                    pstRpmStart->m_pstRPMNextElem = NULL;
                    pstRpmStart->m_pstRPMNextElem =
                        OSAL_Malloc(sizeof(rpm_response_t), __FILE__, __FUNCTION__, __LINE__);

                    if(NULL != pstRpmStart->m_pstRPMNextElem)
                    {
                        pstRpmStart = pstRpmStart->m_pstRPMNextElem;
                        pstRpmStart->m_pstRPMNextElem = NULL;
                    }
                    else
					{
						...
						Fill_BacnetIP_Arguments: MAlloc Failed \r\n");
						#endif
					}
                }

                /* Get next property pointer */
...
#if (defined BACDEL_SER_DS_WP_A || defined BASIC_A_SIDE)
        case SERVICE_SUPPORTED_WRITE_PROPERTY:
			/* no data to be copied as simple ack is received */
            break;
#endif
        ...
        case MAX_BACNET_SERVICES_SUPPORTED:
        {
			...
			#if (STACK_FALLBACK_FOR_RPM)
            else if(true == pstInitQData->m_bRPMError)
            {
                rpm_response_t  *pstRpmStart = NULL;
                rpm_response_t  *pstRpmTemp = NULL;

                pstRpmStart = pstInitQData->m_pstRpmIPArgs;
// Commented code to avoid sending error to application received in RPM fall back
#if 1
                if(NULL == pstRpmStart)
                {
                    /* Allocate memory to Application arguments */
				...
				pstRpmStart->m_eObjectType =
                    ((Bacnet_Multiple_Data_t*)pstInitQData->m_pRpmRequestParam)->m_stServiceData.eObjectType;

				pstRpmStart->m_eObjectProperty =
                    ((Bacnet_Multiple_Data_t*)pstInitQData->m_pRpmRequestParam)->m_stServiceData.eObjectProperty;

                pstRpmStart->m_u32ArrayIndex =
                    ((Bacnet_Multiple_Data_t*)pstInitQData->m_pRpmRequestParam)->m_stServiceData.u32ArrayIndex;

				pstRpmStart->m_u8ArrayIndexPresent =
                    ((Bacnet_Multiple_Data_t*)pstInitQData->m_pRpmRequestParam)->m_stServiceData.bArrIndxPresent;

                /* Set Error class and Error code to -1 to indiacate successful response */
				pstRpmStart->m_eErrorClass =
                    pstRspQueue->m_stProcessData.m_stAPDU.m_u32ErrorClass;

                /* In case of Abort or Reject reception */
                if(PDU_TYPE_ABORT == pstRspQueue->m_stProcessData.m_ePDUType)
					pstRpmStart->m_eErrorCode = pstRspQueue->m_stProcessData.m_stAPDU.m_u32ErrorCode;
                else if(PDU_TYPE_REJECT == pstRspQueue->m_stProcessData.m_ePDUType)
					pstRpmStart->m_eErrorCode = pstRspQueue->m_stProcessData.m_stAPDU.m_u32ErrorCode;
                else
                {
					pstRpmStart->m_eErrorCode =
                        pstRspQueue->m_stProcessData.m_stAPDU.m_u32ErrorCode;
                }
#endif
            }
			#endif /* STACK_FALLBACK_FOR_RPM */
            else
            {
...
BACNET_RETURN_TYPE Validate_Received_Request(bacnetip_arguments_t *pstBACnetArgs)
{
    ...
    case SERVICE_SUPPORTED_WRITE_PROPERTY:
    {
		...
		Wp_A_Display_Data(&pstBACnetArgs->m_stNPDUData.
			m_stAPDUData.m_stServiceChoice.m_stWP_Request);
		#endif

		/* check object type */
        if (pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stWP_Request.m_eObjectType >= MAX_BACNET_OBJECT_TYPE ||
			pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stWP_Request.m_eObjectType < 0)
        {
            eRetVal = BACDEL_INITIATOR_MAX_OBJECT_TYPE_ERROR;
			break;
        }

		/* check object instance number */
        if (pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stWP_Request.m_u32ObjectInstance > BACNET_MAX_INSTANCE)
        {
		...
		/* check property id */
        if (pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stWP_Request.m_eObjectProperty > MAX_BACNET_PROPERTY_ID ||
			pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stWP_Request.m_eObjectProperty < 0)
        {
			eRetVal = BACDEL_INITIATOR_MAX_PROPERTY_ERROR;
			break;
        }

		/* check Priority */
		if(/*(pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_u32Priority
			< BACNET_NO_PRIORITY) ||*/
			pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_u32Priority
			> BACNET_MAX_PRIORITY)
        {
    ...
    case SERVICE_SUPPORTED_READ_PROPERTY:
    {
		#ifdef DISPLAY_PARAMETERS
		/* display service data */
		RP_A_Display_Data(&pstBACnetArgs->m_stNPDUData.
			m_stAPDUData.m_stServiceChoice.m_stRP_Request);
		#endif

		/* check object type */
        if (pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stRP_Request.m_eObjectType >= MAX_BACNET_OBJECT_TYPE ||
			pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stRP_Request.m_eObjectType < 0)
        {
            eRetVal = BACDEL_INITIATOR_MAX_OBJECT_TYPE_ERROR;
			break;
        }
		...
		/* check object instance number */
        if (pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stRP_Request.m_u32ObjectInstance > BACNET_MAX_INSTANCE)
        {
            eRetVal = BACDEL_INITIATOR_MAX_INSTANCE_ERROR;
			break;
        }

		/* check object Property Id */
        if (pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stRP_Request.m_eObjectProperty > MAX_BACNET_PROPERTY_ID ||
			pstBACnetArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
			m_stRP_Request.m_eObjectProperty < 0)
        {
            eRetVal = BACDEL_INITIATOR_MAX_PROPERTY_ERROR;
			break;
        }
    ...
    case SERVICE_SUPPORTED_READ_PROP_MULTIPLE:
    {
        rpm_request_t *pstRPMStruct = &(pstBACnetArgs->m_stNPDUData.
            m_stAPDUData.m_stServiceChoice.m_stRPM_Request);

		#ifdef DISPLAY_PARAMETERS
		/* display service data */
		RPM_A_Display_Data(pstRPMStruct);
		#endif

        while(NULL != pstRPMStruct)
        {
			/* check object type */
			if (pstRPMStruct->m_eObjectType >= MAX_BACNET_OBJECT_TYPE ||
				pstRPMStruct->m_eObjectType < 0)
            {
                eRetVal = BACDEL_INITIATOR_MAX_OBJECT_TYPE_ERROR;
				break;
            }
			...
			if (pstRPMStruct->m_u32ObjectInstance > BACNET_MAX_INSTANCE)
            {
                eRetVal = BACDEL_INITIATOR_MAX_INSTANCE_ERROR;
				break;
            }
			...
			/* check property id */
			if (pstRPMStruct->m_eObjectProperty > MAX_BACNET_PROPERTY_ID ||
				pstRPMStruct->m_eObjectProperty < 0)
            {
                eRetVal = BACDEL_INITIATOR_MAX_PROPERTY_ERROR;
				break;
            }
...
*
* DESCRIPTION
* Function decodes the property value as per data type of property.
* This function returns following things:
* - decoded property value in string format.
* - decoded property value in structure format as a primitive data or list of primitive data.
* - decoded property value in native data type structure format.
* - string length for string format.
* - data type of property.
* - error code and class in case of error.
*
* @param pvData				[in]  Service response data received for decoding.
* @param i32ServiceChoice	[in]  Service type.
* @param pi8PropValStr		[out] Pointer to save property value in string format.
* @param pi32StringLen		[out] Pointer to save property value string length.
* @param pstPropValue		[out] Pointer to save property value in primitive structure format.
* @param ppvPropValue		[out] Pointer to save property value in native data type structure format.
* @param peDataType			[out] Pointer to save property data type.
* @param peErrCode			[out] Pointer to save error code.
* @param peErrClass			[out] Pointer to save erorr class.
*
* @return [out] returns void.
*
**/
void Decode_A_Side_Data(void *pvData,
	int8_t **pi8PropValStr,
	BACNET_PROPERTY_VALUE *pstPropValue,
	int32_t i32ServiceSupported,
	int32_t *pi32StringLen,
	void **ppvPropValue,
	...
	BACNET_ERROR_CLASS *peErrClass)
{
	...
	Decode_A_Side_Data: Entry \r\n");
	#endif

    if(NULL == pi8PropValStr || NULL == pstPropValue || NULL == pi32StringLen
        || NULL == peErrCode || NULL == peErrClass || NULL == peDataType
		|| NULL == ppvPropValue || NULL == pvData)
    {
		#if(defined DEBUG_PRINTF && DL_0)
		Print_DebugMsg(DEBUG_LEVEL0, BACDEL_INVALID_INPUT_PARAMETER, "APDU: \
		Decode_A_Side_Data: Null input pointers. \r\n");
		#endif
        return;
    }
	...
	/* set default values */
	*peErrClass = ERROR_CLASS_PROPERTY;
	*peErrCode = MAX_BACNET_ERROR_CODE;
	*ppvPropValue = NULL;
	*peDataType = BACNET_DT_EMPTY;
	*pi8PropValStr = NULL;
	*pi32StringLen = 0;
	memset(pstPropValue, BACNET_ZERO, sizeof(BACNET_PROPERTY_VALUE));

    /* Check the service type and call decoding of application data */
    switch(i32ServiceSupported)
    {
#if ((defined  BACDEL_SER_DS_RP_A) || (defined BACDEL_SER_DS_RPM_A) || (defined BASIC_A_SIDE))
        case SERVICE_SUPPORTED_READ_PROPERTY :
        case SERVICE_SUPPORTED_READ_PROP_MULTIPLE:
        {
			#ifdef OLD_RP_WP_INTERFACE
            /* Decode the application data for A-side request */
            i32len = BACApp_Decode_Application_Data(
                        ((BACNET_CONF_DATA *)pvData)->pu8PropValueBuffer,
                        (uint32_t)((BACNET_CONF_DATA *)pvData)->i32ApplicationDataLen,
                        &appDataValue, ((BACNET_CONF_DATA *)pvData)->eObjectProperty,
                        ((BACNET_CONF_DATA *)pvData)->eObjectType);

            /* check for error */
            if(BACNET_APPLICATION_TAG_CHARACTER_STRING == appDataValue.m_TagType)
            {
                /* set error code */
				if(Validate_Charstring_Encoding(appDataValue.uValue.m_stCharacter_String.m_u8Encoding,
					appDataValue.uValue.m_stCharacter_String.m_u16CodePage))
                    *peErrCode = ERROR_CODE_CHARACTER_SET_NOT_SUPPORTED;
                else if(i32len < 0)
                    *peErrCode = ERROR_CODE_VALUE_OUT_OF_RANGE;
                if(*peErrCode != MAX_BACNET_ERROR_CODE)
                    i32len = -1; // to free any allocated memory
            }
            else if(BACNET_APPLICATION_TAG_OCTET_STRING == appDataValue.m_TagType &&
                i32len < 0)
            {
                /* set error code */
                *peErrCode = ERROR_CODE_VALUE_OUT_OF_RANGE;
            ...
            else if(BACNET_APPLICATION_TAG_BIT_STRING == appDataValue.m_TagType &&
                i32len < 0)
            {
                /* set error code */
                *peErrCode = ERROR_CODE_VALUE_OUT_OF_RANGE;
            ...
            if (i32len > 0)
            {
                /* Fill in the data value depending upon the Tag type */
                Convert_AppTag_To_String( pi8PropValStr, &appDataValue,
					((BACNET_CONF_DATA *)pvData)->eObjectType,
                    ((BACNET_CONF_DATA *)pvData)->eObjectProperty,
					((BACNET_CONF_DATA *)pvData)->u32ArrayIndex,
					pi32StringLen,
                    ((BACNET_CONF_DATA *)pvData)->bArrIndxPresent,
					false);

                /* Copy Property value into IP Argument for Application */
                if(NULL != pstPropValue)
	                memcpy(pstPropValue, &appDataValue, sizeof(BACNET_PROPERTY_VALUE));
                else
                    /* clear the property values */
                    Clear_Property_Value(appDataValue.pstNextPropVal);
            }
            else
                /* error in decoding so free malloced memories */
                Clear_Property_Value(appDataValue.pstNextPropVal);
			#endif /* */

			#ifdef NEW_RP_WP_INTERFACE
			/* decode property value as per new interface */
			i32len = 0;
			i32len = BACApp_Decode_Data_Type(
				((BACNET_CONF_DATA *)pvData)->pu8PropValueBuffer,
				((BACNET_CONF_DATA *)pvData)->i32ApplicationDataLen,
				((BACNET_CONF_DATA *)pvData)->eObjectProperty,
				((BACNET_CONF_DATA *)pvData)->eObjectType,
				peDataType, ppvPropValue,
				((BACNET_CONF_DATA *)pvData)->u32ArrayIndex,
				((BACNET_CONF_DATA *)pvData)->bArrIndxPresent,
			...
			if(i32len < 0)
			{
				...
				Convert_DT_Error_To_Error_Code(peErrCode, peErrClass, i32len);
				/* clear allocated memories */
				Clear_PropVal_AsPer_DataType(*peDataType, ppvPropValue);
				/* update the data type and pointer value */
...
#ifdef BACDEL_SER_DS_RR_A
        case SERVICE_SUPPORTED_READ_RANGE:
        {
			#ifdef OLD_RP_WP_INTERFACE
            /* Decode the application data for A-side request */
            i32len = BACApp_Decode_Application_Data(
				((readrange_response_t *)pvData)->m_pvPropVal,
				((readrange_response_t *)pvData)->m_u32DataLen,
                &appDataValue,
				((readrange_response_t *)pvData)->m_ePropertyId,
				((readrange_response_t *)pvData)->m_eObjectType);
            ...
            if (i32len > 0)
            {
                /* Fill in the data value depending upon the Tag type */
                Convert_AppTag_To_String( pi8PropValStr, &appDataValue,
					(uint8_t)((readrange_response_t *)pvData)->m_eObjectType,
					((readrange_response_t *)pvData)->m_ePropertyId,
					((readrange_response_t *)pvData)->m_u32ArrayIndex,
					pi32StringLen,
					((readrange_response_t *)pvData)->m_u8ArrayIndexPresent,
					false);
        ...
        default:
        {
			...
			Decode_A_Side_Data : Service is not supported\r\n");
			#endif
        }
        break;
    }//switch ends.
	...
	Decode_A_Side_Data: Exit \r\n");
	#endif
}
#if ((defined  BACDEL_SER_DS_RP_A) || (defined BACDEL_SER_DS_RPM_A) || (defined BACDEL_SER_DS_RR_A))
/***
* Api to fill error response in ip arguments when there is error in decoding
* property value in RP / RPM response due to out_of_range value,
* unsupported_character_set, etc.
**/
void Fill_Error_Response(BACNET_ERROR_CODE eErrCode, BACNET_ERROR_CLASS eErrClass,
		                 uint8_t u8ServiceSupported, void *pvErrResp)
{
    /* local variables */
    ...
    /* switch to service choice */
    switch(u8ServiceSupported)
    {
	#if (defined  BACDEL_SER_DS_RP_A) || (defined BACDEL_SER_DS_RR_A)
    case SERVICE_SUPPORTED_READ_PROPERTY:
	case SERVICE_SUPPORTED_READ_RANGE:
		((error_response_t *)pvErrResp)->m_eErrorClass = eErrClass;
		((error_response_t *)pvErrResp)->m_eErrorCode = eErrCode;
        ((error_response_t *)pvErrResp)->m_i32FirstFailedNo = 0;
        break;
	#endif /* RP-A || RR-A */

	#ifdef BACDEL_SER_DS_RPM_A
    case SERVICE_SUPPORTED_READ_PROP_MULTIPLE:
		((rpm_response_t *)pvErrResp)->m_eErrorClass = eErrClass;
		((rpm_response_t *)pvErrResp)->m_eErrorCode = eErrCode;
        break;
	#endif /* RPM-B */
    default:
        break;
	...
	#if(defined DEBUG_PRINTF && DL_3)
    Print_DebugMsg(DEBUG_LEVEL3, BACDEL_EXIT_MESSAGE, "APDU: \
	Fill_Error_Response: Exit \r\n");
	#endif
}
#endif /* (RP-A || RR-A || RPM-A) */
/************************ end of file **************************/
