	...
	App_Callback_Interface_t fpFunctionPtr)
{
    ...
    switch(pstServiceArgs->m_stNPDUData.m_stAPDUData.m_eServiceSupport)
    {
		...
		case SERVICE_SUPPORTED_READ_PROPERTY:
		{
			...
			/* Set service Choice */
			pstReqParam->m_stAPDU.m_u8ServiceChoice = SERVICE_CONFIRMED_READ_PROPERTY;
		   ...
		   NPDU_Set_Data(&pstReqParam->m_stNPDU, true,
			...
			/* Allocate buffer for RP request data */
			pstRpData = (BACNET_CONF_DATA *)OSAL_Malloc( sizeof(BACNET_CONF_DATA),  __FILE__, __FUNCTION__, __LINE__);
			if( pstRpData == NULL )
			{
				/* No space for initiating request */
				/* Fill initiator error response */
				Fill_Initiator_Response(BACDEL_INITIATOR_MALLOC_ERROR,
					&stInitRet, &pstInitiateInfoQ);
				#ifdef DEBUG_PRINTF
				Print_DebugMsg(DEBUG_LEVEL1,"BACnetStackAPI: BACDEL_Generate_Aside_Request: \
				BACDEL_INITIATOR_MALLOC_ERROR\r\n");
				#endif
				return stInitRet;
			}

			/* Set Read service parameters */
			pstRpData->eObjectType = pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stRP_Request.m_eObjectType;
			pstRpData->u32ObjectInstance = pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stRP_Request.m_u32ObjectInstance;
			pstRpData->eObjectProperty = pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stRP_Request.m_eObjectProperty;
			pstRpData->u32ArrayIndex = pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stRP_Request.m_u32ArrayIndex;
			if(ARRAY_INDEX_PRESENT == pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stRP_Request.m_u8ArrayIndexPresent)
					pstRpData->bArrIndxPresent = ARRAY_INDEX_PRESENT;

			/* Set ServiceRequestData as RP-B service paramaters */
			pstReqParam->m_stAPDU.m_pvServiceRequestData = pstRpData;
		}
		break;
       ...
       case SERVICE_SUPPORTED_WRITE_PROPERTY:
       {
           ...
           BACNET_PROPERTY_VALUE *pstPropValInData = NULL;
           PROP_ACCESS_TYPE eType = -1;
#endif

#ifdef CONSOLE_APPLICATION
		    BACNET_APPLICATION_TAG eTagType = TAG_NOT_SUPPORTED;
           ARRAY_TYPE_SUPPORTED eArrayProperty = 0;

           eArrayProperty = GetPropertyDataType(pstServiceArgs->m_stNPDUData.m_stAPDUData.
                               m_stServiceChoice.m_stWP_Request.m_i32ObjectType,
                               pstServiceArgs->m_stNPDUData.m_stAPDUData.
                               m_stServiceChoice.m_stWP_Request.m_i32ObjectProperty,
                               &eTagType);

           /* Check support Of Data Type for Encoding */
           if( (eTagType != BACNET_APPLICATION_TAG_NULL) &&
               (eTagType != BACNET_APPLICATION_TAG_BOOLEAN) &&
               (eTagType != BACNET_APPLICATION_TAG_UNSIGNED_INT) &&
               (eTagType != BACNET_APPLICATION_TAG_REAL) &&
               (eTagType != BACNET_APPLICATION_TAG_CHARACTER_STRING) &&
               (eTagType != BACNET_APPLICATION_TAG_ENUMERATED) &&
               (eTagType != BACNET_APPLICATION_TAG_BIT_STRING) &&
               (eTagType != BACNET_APPLICATION_TAG_LISTCHAR) &&
               (eTagType != BACNET_APPLICATION_TAG_LISTUNSIGN) )
           {
				/* Application Tag type is not supported in WP-A */
				/* Fill initiator error response */
				Fill_Initiator_Response(BACDEL_INITIATOR_INVALID_PROPERTY_TAG,
					&stInitRet, &pstInitiateInfoQ);
				#if(defined DEBUG_PRINTF && DL_1)
			    Print_DebugMsg(DEBUG_LEVEL1, BACDEL_INITIATOR_INVALID_PROPERTY_TAG, "API: BACDEL_Generate_Aside_Request: \
									Property tag not supported \r\n");
				#endif
               return stInitRet;
           }

           if((eArrayProperty == ARRAY_TYPE_1) &&
               (ARRAY_INDEX_PRESENT != pstServiceArgs->m_stNPDUData.m_stAPDUData.
                   m_stServiceChoice.m_stWP_Request.m_i8ArrayIndexPresent) &&
               (PROP_STATE_TEXT != pstServiceArgs->m_stNPDUData.m_stAPDUData.
                   m_stServiceChoice.m_stWP_Request.m_i32ObjectProperty))
           {
               /* Application Tag type is not supported in WP-A */
				/* Fill initiator error response */
...
#endif
           /* Set service Choice */
           pstReqParam->m_stAPDU.m_u8ServiceChoice = SERVICE_CONFIRMED_WRITE_PROPERTY;
           ...
           /* Allocate buffer for WP request data */
           pstWpData = (BACNET_CONF_DATA *)OSAL_Malloc( sizeof(BACNET_CONF_DATA),  __FILE__, __FUNCTION__, __LINE__);
           if( pstWpData == NULL )
           {
               /* No space for initiating request */
				/* Fill initiator error response */
           ...
           /* Set WP-A service parameters */
           pstWpData->eObjectType = pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stWP_Request.m_eObjectType;
           pstWpData->u32ObjectInstance = pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stWP_Request.m_u32ObjectInstance;
           pstWpData->eObjectProperty = pstServiceArgs->m_stNPDUData.m_stAPDUData.
				m_stServiceChoice.m_stWP_Request.m_eObjectProperty;
           pstWpData->u32ArrayIndex = pstServiceArgs->m_stNPDUData.m_stAPDUData.
               m_stServiceChoice.m_stWP_Request.m_u32ArrayIndex;
           if(ARRAY_INDEX_PRESENT == pstServiceArgs->m_stNPDUData.
				m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_u8ArrayIndexPresent)
                   pstWpData->bArrIndxPresent = ARRAY_INDEX_PRESENT;

           /* Copy priority and Property value */
           pstWpData->u8Priority = (uint8_t)pstServiceArgs->m_stNPDUData.
				m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_u32Priority;

#ifdef OLD_RP_WP_INTERFACE
#ifndef update_access_type
           /* call api to update property access type */
           if(IGNORE_ACCESS_TYPE != pstServiceArgs->m_stNPDUData.m_stAPDUData.
               m_stServiceChoice.m_stWP_Request.m_eAccessType)
           {
               /* set the access type */
               eType = BACDEL_Set_Get_Property_AccessType(pstServiceArgs->m_u32DeviceID,
                   pstWpData->u32ObjectInstance, pstWpData->eObjectType,
                   pstWpData->eObjectProperty, pstServiceArgs->m_stNPDUData.m_stAPDUData.
                   m_stServiceChoice.m_stWP_Request.m_eAccessType, true);

               if(eType != pstServiceArgs->m_stNPDUData.m_stAPDUData.
                   m_stServiceChoice.m_stWP_Request.m_eAccessType)
               {
			        /* Fill initiator error response */
			        Fill_Initiator_Response(BACDEL_INITIATOR_ERROR,
				        &stInitRet, &pstInitiateInfoQ);
					#if(defined DEBUG_PRINTF && DL_1)
			        Print_DebugMsg(DEBUG_LEVEL1, BACDEL_INITIATOR_ERROR, "API: BACDEL_Generate_Aside_Request: \
...
#ifdef OLD_RP_WP_INTERFACE
           pstPropValInData =
               &pstServiceArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_stPropertyValue;

           pstWpData->pvReadPropValue = NULL;
           pstPropVal =
               OSAL_Malloc(sizeof(BACNET_PROPERTY_VALUE), __FILE__,__FUNCTION__,__LINE__);
           ...
           /* attach the value pointer to request data */
           pstWpData->pvReadPropValue = pstPropVal;
           while(NULL != pstPropValInData)
           {
               /* copy the data */
               memcpy(pstPropVal, pstPropValInData, sizeof(BACNET_PROPERTY_VALUE));
               /* move to next input prop val node */
               pstPropValInData = pstPropValInData->pstNextPropVal;
               if(NULL != pstPropValInData)
               {
           ...
           pstPropVal->pstNextPropVal = NULL;
			...
			/* save the data type */
			pstWpData->eData_Type =
				pstServiceArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_eDataType;

			/* call function to copy property value */
			eResponseType = Copy_PropVal_AsPer_DataType(
				pstServiceArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_pvPropVal,
				pstServiceArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_eDataType,
				&pstWpData->pvReadPropValue);

			if(eResponseType != BACDEL_SUCCESS)
			{
				/* Fill initiator error response */
               Fill_Initiator_Response(eResponseType, &stInitRet, &pstInitiateInfoQ);
				#if(defined DEBUG_PRINTF && DL_1)
               Print_DebugMsg(DEBUG_LEVEL1, eResponseType, "API: BACDEL_Generate_Aside_Request: \
               WP_A property value not copied \r\n");
				#endif
               return stInitRet;
			}

			/* reverse bit string value */
			Reverse_BitString_DataType_Value(pstWpData->eData_Type, pstWpData->pvReadPropValue);

#elif defined CONSOLE_APPLICATION
			/* allocate memory */
           pstWpData->pu8PropValueBuffer = OSAL_Malloc(MAX_APPLN_PROP_VAL_STR, __FILE__, __FUNCTION__, __LINE__ );
           if( NULL == pstWpData->pu8PropValueBuffer)
           {
			...
			/* copy the data */
           memcpy(pstWpData->pu8PropValueBuffer,
               pstServiceArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_i8PropValue,
               Strnlen(pstServiceArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.
               m_stWP_Request.m_i8PropValue, sizeof(pstServiceArgs->m_stNPDUData.
               m_stAPDUData.m_stServiceChoice.m_stWP_Request.m_i8PropValue)) );
#endif

           /* Set ServiceRequestData as WP-A service paramaters */
           pstReqParam->m_stAPDU.m_pvServiceRequestData = pstWpData;
       }
       break;
        ...
        case SERVICE_SUPPORTED_READ_PROP_MULTIPLE:
        {
            ...
            rpm_request_t *pstRequestParam = NULL;
            uint8_t u8NewObjId = 1;

            /* Set APDU type and service Choice */
            pstReqParam->m_ePDUType = PDU_TYPE_CONFIRMED_SERVICE_REQUEST;
            pstReqParam->m_stAPDU.m_u8ServiceChoice = SERVICE_CONFIRMED_READ_PROP_MULTIPLE;
            pstRequestParam =
                &(pstServiceArgs->m_stNPDUData.m_stAPDUData.m_stServiceChoice.m_stRPM_Request);

            /* Set NPDU parameters */
//            NPDU_Set_Data(&pstReqParam->m_stNPDU, true, pstServiceArgs->m_ePriority, false, 0);<@>

            /* Loop for all paramters are enterd */
            while(1)
            {
				if(pstReqParam->m_stAPDU.m_pvServiceRequestData == NULL)
				{
					/* Allocate buffer for read property data */
					pstReqParam->m_stAPDU.m_pvServiceRequestData = (Bacnet_Multiple_Data_t *)OSAL_Malloc
                    (sizeof(Bacnet_Multiple_Data_t),  __FILE__, __FUNCTION__, __LINE__);
                    if(NULL == pstReqParam->m_stAPDU.m_pvServiceRequestData)
                    {
                        /* Fill initiator error response */
//				        Fill_Initiator_Response(BACDEL_INITIATOR_MALLOC_ERROR, <@>
//					        &stInitRet, &pstInitiateInfoQ);
					...
					pstRpmData = pstReqParam->m_stAPDU.m_pvServiceRequestData;
				}
				else
				{
					pstRpmData->m_pstMDNext = (Bacnet_Multiple_Data_t *)OSAL_Malloc
                    (sizeof(Bacnet_Multiple_Data_t),  __FILE__, __FUNCTION__, __LINE__);
...
//					        &stInitRet, &pstInitiateInfoQ);
                ...
                /* Set Read service parameters */
				pstRpmData->m_stServiceData.eObjectType = pstRequestParam->m_eObjectType;
				pstRpmData->m_stServiceData.u32ObjectInstance = pstRequestParam->m_u32ObjectInstance;
				pstRpmData->m_stServiceData.eObjectProperty = pstRequestParam->m_eObjectProperty;
                pstRpmData->m_stServiceData.u32ArrayIndex = pstRequestParam->m_u32ArrayIndex;
				if(ARRAY_INDEX_PRESENT == pstRequestParam->m_u8ArrayIndexPresent)
                    pstRpmData->m_stServiceData.bArrIndxPresent = ARRAY_INDEX_PRESENT;

                pstRpmData->m_stServiceData.u8FillObjId = u8NewObjId;

                /* Check for next property Id for Read is added or not */
                if(NULL != ((rpm_request_t *)pstRequestParam)->m_pstRPMNextElem)
                {
					if((pstRequestParam->m_eObjectType ==
						pstRequestParam->m_pstRPMNextElem->m_eObjectType) &&
						(pstRequestParam->m_u32ObjectInstance ==
						pstRequestParam->m_pstRPMNextElem->m_u32ObjectInstance))
                        u8NewObjId = 0;
                    else
                        u8NewObjId = 1;

                    pstRequestParam = ((rpm_request_t *)pstRequestParam)->m_pstRPMNextElem;
                }
                else
